...
class PlayerViewModel(private val tracksSearchApi: TracksSearchApi): ViewModel() {
    ...
    fun searchTrack(trackId: String) {
        tracksSearchApi.searchTracks(trackId).enqueue(object : Callback<TrackSearchResponse> {
            override fun onResponse(
                call: Call<TrackSearchResponse>,
                response: Response<TrackSearchResponse>
            ) {
                _trackLiveData.value = PlayerState.TrackLoaded(response.body()?.results?.first() ?: return)
            }
